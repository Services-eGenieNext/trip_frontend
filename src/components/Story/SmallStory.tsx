...
const SmallStory = ({positioning, item}:ISmallStory) => {

    const description = item?.location_id ? item?.description : item?.editorial_summary?.overview

    return description && description.trim() !== '' ? (
        <div className={`sm-width`}>
            <div className={`${positioning == "inline" ? "grid grid-cols-12 items-center" : "flex flex-col"} gap-4 my-10 md:my-20 p-10 rounded-xl bg-white ${styles.small_story_wrapper}`}>
